...
!jd-
           f_fswdn     = 'mxxxx', f_fswdn_ai   = 'mxxxx', &
           f_fswup     = 'mxxxx', f_fswup_ai   = 'mxxxx',  &
           f_flwdn     = 'mxxxx', f_flwdn_ai    = 'mxxxx', &
!jd-
...
!jd
      ...
      call broadcast_scalar (f_fswdn_ai, master_task)
      call broadcast_scalar (f_fswup, master_task)
      call broadcast_scalar (f_fswup_ai, master_task)
      call broadcast_scalar (f_flwdn, master_task)
      call broadcast_scalar (f_flwdn_ai, master_task)
!jd
...
!jd-newdef-start
      ...
      if (f_fswdn_ai(1:1) /= 'x') &
         call define_hist_field(n_fswdn_ai,"fswdn_ai","W/m^2",tstr, tcstr, &
             "down solar flux weighted with respect to ice area",          &
             "positive downward", c1, c0,                                  &
             ns1, f_fswdn_ai)
      ...
      if (f_fswup_ai(1:1) /= 'x') &
         call define_hist_field(n_fswup_ai,"fswup_ai","W/m^2",tstr, tcstr, &
             "upward solar flux weighted with sea ice area",                                    &
             "positive down", c1, c0,                            &
             ns1, f_fswup_ai)
      ...
      if (f_flwdn_ai(1:1) /= 'x') &
         call define_hist_field(n_flwdn_ai,"flwdn_ai","W/m^2",tstr, tcstr, &
             "down longwave flux weighted with respect to ice area",       &
             "positive downward", c1, c0,                            &
             ns1, f_flwdn_ai)
!jd-newdef-stop
...
!jd-newdef-start
         if (f_fswdn_ai(1:1) /= 'x') &
            call accum_hist_field(n_fswdn_ai, iblk, fsw(:,:,iblk)*workb(:,:))
         if (f_fswup(1:1) /= 'x') &
            call accum_hist_field(n_fswup, iblk, &
                 -(fsw(:,:,iblk)-fswabs(:,:,iblk)))            ! Positive down 
         if (f_fswup_ai(1:1) /= 'x') &
            call accum_hist_field(n_fswup_ai, iblk, &
                 -(fsw(:,:,iblk)-fswabs(:,:,iblk))*workb(:,:)) ! Positive down 
         if (f_flwdn_ai(1:1) /= 'x') &
            call accum_hist_field(n_flwdn_ai, iblk, flw(:,:,iblk)*workb(:,:) )
!jd-newdef-stop
         if (f_flwdn(1:1) /= 'x') &
