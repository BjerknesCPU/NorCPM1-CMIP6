...
! SVN $URL: https://svn-ccsm-models.cgd.ucar.edu/drv/seq_mct/branch_tags/cesm1_0_4_rel_tags/cesm1_0_4_n01_drvseq3_1_48/shr/seq_infodata_mod.F90 $
!===============================================================================
!BOP ===========================================================================
!
...
!-------------------------------------------------------------------------------
    ...
    if (seq_comm_iamroot(ID)) then
       ...
       infodata%turb_flux_scheme      = turb_flux_scheme 
...
SUBROUTINE seq_infodata_GetData( infodata, case_name, case_desc, timing_dir,  &
           ...
           eps_agrid, eps_aarea, eps_omask, eps_ogrid, eps_oarea,turb_flux_scheme )
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   integer             ,optional, intent(OUT) :: turb_flux_scheme 
...
!-------------------------------------------------------------------------------
    ...
    if ( present(turb_flux_scheme)      ) turb_flux_scheme      = infodata%turb_flux_scheme
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   integer,             optional, intent(IN) :: turb_flux_scheme 
...
!-------------------------------------------------------------------------------
    ...
    if ( present(turb_flux_scheme)      ) infodata%turb_flux_scheme      = turb_flux_scheme 
...
!-------------------------------------------------------------------------------
    ...
    call shr_mpi_bcast(infodata%turb_flux_scheme,             mpicom)
...
!    if (loglevel > 0) then
     ...
     elseif (trim(infodata%orb_mode) == trim(seq_infodata_orb_variable_year)) then
       ...
       write(logunit,F0S) subname,'turb_flux_scheme         = ', infodata%turb_flux_scheme 
