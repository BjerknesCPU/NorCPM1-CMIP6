...
!-------------------------------------------------------------------------------
   ...
   use shr_flux_mod,      only: shr_map_docoare 
...
subroutine ccsm_pre_init()
   ...
   integer :: myid, mpi_err, new_comm, rank_local, mppwidth, ierr, nu, npes
   integer :: mem1, mem 
   ...
   !--- set up ensemble mode, if necessary --- 
   call mpi_comm_size( mpi_comm_world, mppwidth, mpi_err)
   call mpi_comm_rank( mpi_comm_world, myid, mpi_err )   
   ! Read namelist if it exists
   if (myid.eq.0) then 
     nu = shr_file_getUnit()
     open(nu,file='drv_in',status='old', iostat=ierr)
     if (ierr == 0) then
        read(nu, nml=ccsm_pes, iostat=ierr)
        close(nu)
     else 
        write(*,*) 'ERROR: cannot open drv_in' 
        stop
     end if
     call shr_file_freeUnit(nu)
     npes=ocn_ntasks+ocn_rootpe
     write(*,*) 'INFO: npes=',npes  
   endif 
   call mpi_bcast(npes,1,mpi_integer,0,mpi_comm_world,mpi_err)
   !
   if (mppwidth.ge.2*npes) then 
     ...
     call getcwd(rundir) 
     read(rundir(LEN_TRIM(rundir)-5:LEN_TRIM(rundir)-4),'(i2.2)') mem1
     write(rundir(LEN_TRIM(rundir)-5:LEN_TRIM(rundir)-4),'(i2.2)') mod(mem+mem1,100)
     if (mem.gt.0) then
       write(*,*) 'INFO: member ',mod(mem+mem1,100),', change run-dir to',trim(rundir)
       call chdir(TRIM(rundir))
     endif
     !--- create intra-member communicator --- 
     call mpi_comm_split(mpi_comm_world,mem,myid,new_comm,mpi_err)
     Global_Comm=new_comm 
     !--- create inter-member communicator --- 
     rank_local=myid-mem*npes
     call mpi_comm_split(mpi_comm_world,rank_local,myid,comm_enscomp,mpi_err)
     write(*,*) 'INFO: member,myid,rank_local,global_comm,new_comm=',mem,myid,rank_local,global_comm,new_comm
   else 
...
subroutine ccsm_init()
 ...
 106  format( A, f23.12)
   ...
   call seq_infodata_GetData(infodata, shr_map_dopole=shr_map_dopole, turb_flux_scheme=turb_flux_scheme)
   call shr_map_setDopole(shr_map_dopole)
   call shr_map_docoare(turb_flux_scheme)
   !-----------------------------------------------------------------------------
