...
! interpolate to current vertical grid and obtain alpha
  ...
  if (masterproc) then
     ...
     do k=0,pver
        write (iulog,fmt='(i4,1e12.5,1f10.2)') k,pref_edge(k+1),alpha(k)*86400.
     enddo
  endif
...
! Determine the bounds of the background and orographic stress regions
    ...
    do k = 0, pver
       ...
       if (pref_edge(k+1) .lt. 50000._r8) kbotbg  = k    ! spectrum source at 500 mb
!+ Mods for Beres04
       if (pref_edge(k+1) .lt. 10000.) bkbotbg  = k+1    ! beres spectrum source at 100 mb
       if (pref_edge(k+1) .lt. 70000.) k700    = k+1     ! 700 mb index
!- Mods for Beres04
    end do
    ktoporo = 0
...
!
! Get value for nbot_molec, bottom level where molecular diffusion applied
!
    call phys_getopts( nbot_molec_out = nbot_molec )
...
!------------------------------Arguments--------------------------------
    ...
    use physconst,        only: cpairv, kmcnd              ! Constituent dependent physconst variables (WACCM-X)
    ...
    real(r8), intent(in) :: kvt_in(pcols,0:pver)  ! input molecular kinematic conductivity
...
!--------------------------------------------------------
    kvtt(:ncol,0:pver) = kvt_in(:ncol,0:pver)

    do k = 2, nbot_molec
      kvtt(:ncol,k-1) = kvt_in(:ncol,k-1) / (cpairv(:ncol,k,lchnk)+cpairv(:ncol,k-1,lchnk)) * 2.
    enddo
    kvtt(:ncol,0) = kvt_in(:ncol,0) / (1.5*cpairv(:ncol,1,lchnk)-0.5*cpairv(:ncol,2,lchnk)) 		   
