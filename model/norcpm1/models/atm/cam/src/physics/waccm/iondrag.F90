...
module iondrag
  ...
  use phys_buffer  ,only: pbuf_get_fld_idx, pbuf_fld, pbuf_size_max 
...
!-----------------------------------------------------------------------
    ...
    call pbuf_add('PedConduct', 'physpkg', 1,pver,1, PedConduct_idx) ! Pedersen Conductivity 
    call pbuf_add('HallConduct', 'physpkg', 1,pver,1, HallConduct_idx) ! Hall Conductivity 
...
!================================================================================================
  ...
  subroutine iondrag_init( pref_mid )
    ...
    ue_idx = pbuf_get_fld_idx( 'UE' )
    ve_idx = pbuf_get_fld_idx( 'VE' )
  ...
  subroutine iondrag_calc_ions( lchnk, ncol, state, ptend, pbuf, delt )
    ...
    if ( elec_slvd ) then
      q_elec => pbuf(slvd_pbf_ndx)%fld_ptr(1,:,:,lchnk,id_elec)
    else
      q_elec => state%q(:,:,id_elec)
    endif
    if ( xo1_slvd ) then
      q_xo1 => pbuf(slvd_pbf_ndx)%fld_ptr(1,:,:,lchnk,id_xo1)
    else
      q_xo1 => state%q(:,:,id_xo1)
    endif
    if ( xo2_slvd ) then
      q_xo2 => pbuf(slvd_pbf_ndx)%fld_ptr(1,:,:,lchnk,id_xo2)
    else
      q_xo2 => state%q(:,:,id_xo2)
    endif
    if ( o2p_slvd ) then
      q_o2p => pbuf(slvd_pbf_ndx)%fld_ptr(1,:,:,lchnk,id_o2p)
    else
      q_o2p => state%q(:,:,id_o2p)
    endif
    if ( op_slvd ) then
      q_op => pbuf(slvd_pbf_ndx)%fld_ptr(1,:,:,lchnk,id_op)
    else
      q_op => state%q(:,:,id_op)
    endif
    if ( nop_slvd ) then
      q_nop => pbuf(slvd_pbf_ndx)%fld_ptr(1,:,:,lchnk,id_nop)
    else
    ...
    if ( waccmx_is('ionosphere') ) then 
      pbuf(PedConduct_idx)%fld_ptr(1,1:ncol,1:pver,lchnk,1) = sigma_ped(1:ncol,1:pver)
      pbuf(HallConduct_idx)%fld_ptr(1,1:ncol,1:pver,lchnk,1) = sigma_hall(1:ncol,1:pver)
    endif
  ...
  subroutine iondrag_tend( lchnk, ncol, state, ptend, pbuf, &
    ...
    ue     => pbuf(ue_idx)%fld_ptr(1,1:pcols,1,lchnk,1)
    ve     => pbuf(ve_idx)%fld_ptr(1,1:pcols,1,lchnk,1)
  ...
  subroutine jouleheat_tend( lchnk, ncol, state, ptend, pbuf, &
    ...
    if( lchnk == 25 ) then
       ...
       write(iulog,*) 'jouleheat_tend: dt,t,max% dt/t = ',qjoule(i,k)/cpairv(i,k,lchnk),state%t(i,k),max_q,' @ lchnk,i,k = ',lchnk,max_ind(:)    
