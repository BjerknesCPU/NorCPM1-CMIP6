...
!-----------------------------------------------------------------------
    ...
    call addfld('EMIS      ',    '1   ', 1,    'I', 'Cloud longwave emissivity',phys_decomp)
    ...
    if (dosw .or. dolw) then
       ...
       if (dosw) then
          ...
          do icall = N_DIAG, 0, -1
              ...
              if (active_calls(icall)) then
                  ...
                  call rad_cnst_get_gas(icall,'H2O', state, pbuf, sp_hum)
                  ! Get ozone mass mixing ratio.
                  call rad_cnst_get_gas(icall,'O3',  state, pbuf, o3)
                  ! Get CO2 mass mixing ratio and compute column mean values
                  call rad_cnst_get_gas(icall,'CO2', state, pbuf, co2)
                  ! Get N2O mass mixing ratio
                  call rad_cnst_get_gas(icall,'N2O', state, pbuf, n2o)
                  ! Get CH4 mass mixing ratio
                  call rad_cnst_get_gas(icall,'CH4', state, pbuf, ch4)
                  ! Get O2  mass mixing ratio
                  call rad_cnst_get_gas(icall,'O2', state, pbuf, o2)
	  ...
	  ! add fillvalue for night columns
       ...
       if (dolw) then
          ...
          do icall = N_DIAG, 0, -1
              ...
              if (active_calls(icall)) then
                  ...
                  call rad_cnst_get_gas(icall,'CFC11', state, pbuf, cfc11)
                  call rad_cnst_get_gas(icall,'CFC12', state, pbuf, cfc12)
       ...
       if (docosp) then
	      ...
	      call cospsimulator_intr_run(state, pbuf, cam_in, emis, coszrs, cld_swtau_in=cld_tau(rrtmg_sw_cloudsim_band,:,:),&
