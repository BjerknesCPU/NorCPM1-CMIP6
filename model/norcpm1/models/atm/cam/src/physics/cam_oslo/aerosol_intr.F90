...
#endif
#ifdef AERLIFE
    use genaero_intr, only: genaero_wet_intr
#endif
!-----------------------------------------------------------------------
    implicit none
!-----------------------------------------------------------------------
...
#ifdef AERLIFE
       call genaero_wet_intr (state, ptend, nstep, dt, lat, clat, cme, prain, &
            evapr, cldv, cldc, cldn, fracis, calday, cmfdqr, conicw, rainmr, &
lo3,lh2o2,wetdepflx)
#endif
...
#ifdef AERLIFE
    use genaero_intr, only: genaero_drydep_intr
#endif
...
#ifdef AERLIFE
      call genaero_drydep_intr (state, ptend, wvflx, dt, lat, clat, &
         fsds, obklen, ts, ustar, prect, snowh, pblh, hflx, mon, landfrac, &
         icefrac, ocnfrac,cam_in%fv, cam_in%ram1,cam_out)
#endif
    return
