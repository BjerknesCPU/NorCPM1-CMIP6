          ...
          do i = 1,ncol
             if (cloud(i,k) > 0._r8 .and. cldice(i,k) > 0._r8) then
                ...
                if (iciwc > lbound) then ! need this because of log10
                   ...
                   !          Average - 
                !!$             vfall = 122.63_r8 + 44.111_r8*logiwc + 4.2144_r8*logiwc**2
                else
                   vfall = 0._r8
                end if
...
!===============================================================================
  subroutine cld_sediment_tend (ncol, dtime  ,               &
       pint   , pmid   , pdel   , t      ,                   &
#ifdef DIRIND
       cloud  , cldliq , cldlix , cldice , pvliq  , pvice  ,          &
       liqtend, lixtend, icetend, wvtend , htend  , sfliq  , sfice   )
#else
...
#ifdef DIRIND
    real(r8), intent(in)  :: cldlix(pcols,pver)        ! aerosol-calculated cloud liquid water (kg/kg)
#endif
...
#ifdef DIRIND
    real(r8), intent(out) :: lixtend(pcols,pver)       ! aerosol-calculated liquid condensate tend
#endif
...
#ifdef DIRIND
    real(r8) :: fxlix(pcols,pverp)                     ! fluxes at the interfaces, aerosol-calculated liquid (positive = down)
#endif
...
#ifdef DIRIND
    lixtend(:ncol,:) = 0._r8 ! condensate tend (aerosol-calculated liquid)
#endif
...
#ifdef DIRIND
    call getflx(ncol, pint, cldlix, pvliq, dtime, fxlix) !use same fall velocity
#endif
...
#ifdef DIRIND
       fxlix(i,1) = 0
       fxlix(i,pverp) = cldlix(i,pver) * pvliq(i,pverp) * dtime
#endif
    end do
...
#ifdef DIRIND
       fxlix(:ncol,k) = max(0._r8, fxlix(:ncol,k))
#endif
    end do
...
#ifdef DIRIND
          fxlix(i,k+1) = min( fxlix(i,k+1), mxsedfac * cldlix(i,k) * pdel(i,k) )
#endif
...
#ifdef DIRIND
          lixtend(i,k)  = (fxlix(i,k)*cldovrl - fxlix(i,k+1)) / (dtime * pdel(i,k))
#endif
       end do
    end do
