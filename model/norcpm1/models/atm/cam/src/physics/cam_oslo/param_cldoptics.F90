...
#ifdef DIRIND
   integer :: icldlx        ! offline cloud liquid water index
#endif
...
#ifdef DIRIND
    call addfld ('ICLDTWX ','gram/m2 ',pver, 'A','In-cloud cloud total water path (liquid and ice)'&
      ,phys_decomp, sampling_seq='rad_lwsw')
    call addfld ('TGCLDLWX','gram/m2 ',1,    'A','Offline grid-box cloud liquid water path',phys_decomp, sampling_seq='rad_lwsw')
#endif
...
#ifdef DIRIND
    call add_default ('ICLDTWX ', 1, ' ')
    call add_default ('TGCLDLWX', 1, ' ')
#endif
...
#ifdef DIRIND
    real(r8), intent(out) :: cliqwx(pcols,pver)      ! in-cloud cloud liquid water path
    real(r8), intent(out) :: emisx(pcols,pver)       ! cloud emissivity
    real(r8) :: cwx   (pcols,pver)                   ! in-cloud cloud (total) water path
    real(r8) :: gliqwx(pcols,pver)                   ! grid-box cloud liquid water path
    real(r8) :: tgliqwx(pcols)                       ! Vertically integrated liquid water path
#endif
...
#ifdef DIRIND
          gliqwx(i,k) = state%q(i,k,icldlx)*state%pdel(i,k)/gravit*1000.0_r8  ! Grid box liquid water path.
#endif
...
#ifdef DIRIND
          cliqwx(i,k) = gliqwx(i,k) / max(0.01_r8,cldn(i,k))                 ! In-cloud liquid water path.
#endif
...
#ifdef DIRIND
          tgliqwx(i)  = tgliqwx(i) + gliqwx(i,k)
#endif
       end do
    end do
...
#ifdef DIRIND
    cwx(:ncol,:pver) = cicewp(:ncol,:pver) + cliqwx(:ncol,:pver) 
#endif
...
! Cloud emissivity.
#ifdef DIRIND
    call cldems(lchnk, ncol, cwp, cwx, ficemr, rei, emis, emisx, cldtau)
#else
...
#ifdef DIRIND
    call outfld('TGCLDLWX',tgliqwx, pcols,lchnk)
    call outfld('ICLDTWX' ,cwx    , pcols,lchnk)
#endif
