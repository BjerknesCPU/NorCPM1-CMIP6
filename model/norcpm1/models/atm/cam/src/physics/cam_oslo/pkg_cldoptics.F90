...
!===============================================================================
#ifdef DIRIND
  subroutine cldems(lchnk   ,ncol   ,clwp,clwx,fice    ,rei   ,emis,emisx,cldtau)
#else
...
#ifdef DIRIND
    real(r8), intent(in) :: clwx(pcols,pver)       ! aerosol-calculated offline cloud liquid water path (g/m**2)
#endif
...
#ifdef DIRIND
    real(r8), intent(out) :: emisx(pcols,pver)      ! aerosol-calculated offline cloud emissivity (fraction)
#endif
...
#ifdef DIRIND
          emisx(i,k) = 1._r8 - exp(-1.66_r8*kabs*clwx(i,k))
#endif
    ...
    do k=1,pver
       do i=1,ncol
          ...
          re(i,k) = retab(index)*(1._r8-corr)		&
               +retab(index+1)*corr
          !           re(i,k) = max(min(re(i,k),30._r8),10._r8)
       end do
    end do
    !
    return
