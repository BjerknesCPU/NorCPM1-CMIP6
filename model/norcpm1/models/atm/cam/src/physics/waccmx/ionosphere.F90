...
!---------------------------------------------------------------------------------
  ...
  use phys_buffer,    only : pbuf_size_max, pbuf_fld, pbuf_add, pbuf_get_fld_idx !! Needed to get variables from physics buffer
...
!-----------------------------------------------------------------------
    ...
    call pbuf_add('ElecTemp', 'global', 1,pver,1, idx) ! Electron temperature (global so can write to history files) 

    call pbuf_add('IonTemp', 'global', 1,pver,1, idx) ! Ion temperature (global so can write to history files)
...
!------------------------------Arguments--------------------------------
    ...
    type(pbuf_fld),      intent(inout) :: pbuf(pbuf_size_max) ! physics buffer
...
!---------------------------------------------------------------------------------
    indxUe = pbuf_get_fld_idx( 'UE' )
    indxVe = pbuf_get_fld_idx( 'VE' )
    indxWe = pbuf_get_fld_idx( 'VE' )
    ue     => pbuf(indxUe)%fld_ptr(1,1:pcols,1,lchnk,1)
    ve     => pbuf(indxVe)%fld_ptr(1,1:pcols,1,lchnk,1)
    we     => pbuf(indxWe)%fld_ptr(1,1:pcols,1,lchnk,1)
...
!------------------------------------------------------------------------------------
    indxIR = pbuf_get_fld_idx( 'IonRates' )
    ionRates => pbuf(indxIR)%fld_ptr(1,1:pcols,1:pver,lchnk,1:nIonRates)
...
!-------------------------------------------------------------------------------------------
    indxAIPRS = pbuf_get_fld_idx( 'AurIPRateSum' )
    aurIPRateSum => pbuf(indxAIPRS)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
...
!-------------------------------------------------------------------------------
    indxSP = pbuf_get_fld_idx( 'PedConduct' )
    indxSH = pbuf_get_fld_idx( 'HallConduct' )
    sigma_ped  => pbuf(indxSP)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    sigma_hall => pbuf(indxSH)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
...
!-------------------------------------------------------------------------------------------------------------------
    indxTe = pbuf_get_fld_idx( 'ElecTemp' )
    tE => pbuf(indxTe)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    indxTi = pbuf_get_fld_idx( 'IonTemp' )
    ti => pbuf(indxTi)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
...
!------------------------------------------------------------------
    pbuf(indxTe)%fld_ptr(1,1:ncol,1:pver,lchnk,1) = tE(1:ncol,1:pver)
    pbuf(indxTi)%fld_ptr(1,1:ncol,1:pver,lchnk,1) = ti(1:ncol,1:pver)
