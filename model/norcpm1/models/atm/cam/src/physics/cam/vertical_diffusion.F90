  ...
  subroutine vd_register()
    ...
    call pbuf_add( 'kvt',      'global',  1,  pverp,  pbuf_times,  kvt_idx ) 
  ...
  subroutine vertical_diffusion_init()
    ...
    !--------------------------------------------------------------------------
    ! In WACCM-X mode, additional variables needed for molecular diffusion
    !--------------------------------------------------------------------------
    ...
    !----------------------------------------------------------------------------------------
    ! Get top and bottom molecular diffusion limits
    !----------------------------------------------------------------------------------------
    call phys_getopts( do_molec_diff_out = do_molec_diff, ntop_molec_out = ntop_molec, nbot_molec_out = nbot_molec )
    ...
    !----------------------------------------------------------------------------------------
    ! Get top and bottom eddy diffusion limits
    !----------------------------------------------------------------------------------------
    call phys_getopts( ntop_eddy_out = ntop_eddy, nbot_eddy_out = nbot_eddy )
    ...
    case ( 'HB', 'HBR' )
        ...
        call init_hb_diff( gravit, cpair, rair, zvir, ntop_eddy, nbot_eddy, pref_mid, karman, eddy_scheme )
    end select
    ...
    call init_vdiff( r8, pcnst, rair, gravit, fieldlist_wet, fieldlist_dry, fieldlist_molec, errstring )
...
!         if( k == lwaterptr_amode(m) ) go to 20
  ...
  ! The commented 'only' limiter from the following line acommodates broken pgf90 v.5.1.6
    ...
    real(r8) :: kvt(pcols,pverp)                                    ! Molecular kinematic conductivity for temperature [  ]
  ...
  ! All variables are modified by vertical diffusion
    ...
    !  and set calc_kvt to true for calculation of molecular diffusivity
    !------------------------------------------------------------------------
  ...
  ! Note that the output 'tauresx,tauresy' from below subroutines are fully implicit ones.
    ...
    if( errstring .ne. '' ) call endrun(errstring)
    if( do_molec_diff ) then
       pbuf(kvt_idx)%fld_ptr(1,1:ncol,1:pverp,lchnk,time_index) = kvt(:ncol,:)
    endif
