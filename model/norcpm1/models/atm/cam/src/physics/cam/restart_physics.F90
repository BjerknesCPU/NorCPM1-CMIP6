module restart_physics
  ...
  use buffer,           only: pblht, tpert, qpert, initialize_buffer
...
!
  ...
  CONTAINS
    subroutine init_restart_physics ( File, cam_out, hdimids)
    use cam_pio_utils,       only: fillvalue
    use phys_buffer,         only: pbuf_init_restart
    ...
    call pbuf_init_restart(File, hdimids, hdim1_d, hdim2_d, pver_id,pverp_id)
...
!
  ...
  subroutine write_restart_physics (File, cam_out)
	 ...
	 do i=begchunk,endchunk
         ...
         call get_phys_decomp(iodesc, 1,pcnst,1,pio_double)
...
!#######################################################################
    ...
    subroutine read_restart_physics (File, cam_out)
