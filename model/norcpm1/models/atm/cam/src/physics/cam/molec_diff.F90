  ...
  ! ***** FAKE THIS FOR NOW USING MOLECULAR WEIGHT OF DRY AIR FOR ALL TRACERS ****
    ...
    if ( waccmx_is('ionosphere') .or. waccmx_is('neutral') ) then 
      ...
      if ( ierr /= 0 ) call endrun('molec_diff: allocate failed in init_molec_diff')
      alphath(:ncnst) = 0._r8
  ...
  integer function compute_molec_diff( lchnk             ,                                                                                       &
    ...
    real(r8), intent(inout) :: kvt(pcols,pver+1)         ! Kinematic molecular conductivity
  ...
  ! This is a key part of the code.  For WACCM-X, use constituent dependent molecular viscosity and conductivity
    ...
    !
    !  Set diffusivities for momentum and temperature to zero for parts of columns below/above molecular diffusion region
    !
    kvt(:ncol,nbot_molec+1:pver+1) = 0._r8
    if ( ntop_molec > 1 ) kvt(:ncol,1:ntop_molec-1) = 0._r8
  ...
  ! Compute difference between scale heights of constituent and dry air
    ...
    if( fixed_ubc ) then
      if ( waccmx_is('ionosphere') .or. waccmx_is('neutral') ) then     
       ...
       wrk0(:ncol) = (kv(:ncol,ntop_molec) + kmq(:ncol,ntop_molec)) * tmpi(:ncol,ntop_molec)/2._r8   ! /2. is to extrapolate/(pmid(1)-pint(1)) to /(pmid(1)-pmid(0))
