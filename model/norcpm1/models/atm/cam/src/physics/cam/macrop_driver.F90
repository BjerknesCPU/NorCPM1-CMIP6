...
! For purposes of convective ql.
 ...
 ! Compute net stratus fraction using maximum over-lapping assumption
   ...
   ! Here 'state_eq' is the equlibrium state after macrophysics
   ...
   mr_ccliq(1:ncol,1:pver)=0._r8   !! not seen by radiation, so setting to 0 
   mr_ccice(1:ncol,1:pver)=0._r8   !! not seen by radiation, so setting to 0
   ...
   do i=1,ncol
	...
	mr_lsliq(i,k) = state1%q(i,k,ixcldliq)
	mr_lsice(i,k) = state1%q(i,k,ixcldice)
      else
	mr_lsliq(i,k) = 0._r8
	mr_lsice(i,k) = 0._r8
      end if
