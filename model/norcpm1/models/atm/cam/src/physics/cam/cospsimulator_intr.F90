...
#else
   real(r8),parameter :: R_UNDEF = -1.0E30
#endif
...
!added some more sizes to physics buffer in stratiform.F90 for COSP inputs
   if( cam_physpkg_is('cam5') ) then
     ifld = pbuf_get_fld_idx('LS_REFFRAIN')
     ls_reffrain  => pbuf(ifld)%fld_ptr(1,1:pcols,1:pver,lchnk, 1)
     ifld = pbuf_get_fld_idx('LS_REFFSNOW')
     ls_reffsnow  => pbuf(ifld)%fld_ptr(1,1:pcols,1:pver,lchnk, 1)  
     ifld = pbuf_get_fld_idx('CV_REFFLIQ')
     cv_reffliq  => pbuf(ifld)%fld_ptr(1,1:pcols,1:pver,lchnk, 1)
     ifld = pbuf_get_fld_idx('CV_REFFICE')
     cv_reffice  => pbuf(ifld)%fld_ptr(1,1:pcols,1:pver,lchnk, 1) 
   end if
   ...
   elseif( cam_physpkg_is('cam5') ) then
      ...
      ifld = pbuf_get_fld_idx('SH_CLDLIQ1')
      sh_cldliq  => pbuf(ifld)%fld_ptr(1,1:pcols,1:pver,lchnk, 1)
      ifld = pbuf_get_fld_idx('SH_CLDICE1')
      sh_cldice  => pbuf(ifld)%fld_ptr(1,1:pcols,1:pver,lchnk, 1)
   end if
