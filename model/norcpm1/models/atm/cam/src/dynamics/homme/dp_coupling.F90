...
module dp_coupling
  ...
  use element_mod,    only: element_t
  use control_mod,    only: tstep_type, smooth_phis_numcycle
  use abortutils,     only: endrun
...
! LOCAL VARIABLES
    ...
    if( iam .lt. par%nprocs) then
       ...
       tl_f = TimeLevel%nm1  ! process split physics (with leapfrog)
       if (tstep_type==1) tl_f = TimeLevel%n0  ! time split physics (with forward-in-time RK)
...
!
!$omp parallel do private (lchnk, ncol, k, i, zvirv)
    do lchnk = begchunk,endchunk
