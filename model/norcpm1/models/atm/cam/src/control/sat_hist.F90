...
!-------------------------------------------------------------------------------
  subroutine get_indices( lats, lons, ncols, col_ndxs, chk_ndxs, fdyn_ndxs, ldyn_ndxs, phs_owners, dyn_owners, mlats,mlons )
    ...
    use dyn_grid,  only: dyn_grid_find_gcol
    ...
    do i = 1,ncols
       ...
       call dyn_grid_find_gcol( lat, lon, dyn_owners(i), idyn1, idyn2, rlat, rlon ) 

       if (debug) then
          call phys_grid_find_col( lat, lon, phs_owners(i), ichk, icol )
          if ( iam==phs_owners(i) ) then
             tlat = get_rlat_p( ichk, icol )*rad2deg
             tlon = get_rlon_p( ichk, icol )*rad2deg
             if ( abs(tlat-rlat)>1.e-3 ) then
                write(*,'(a,3f20.12)') ' lat, tlat, rlat = ', lat, tlat, rlat
                write(*,'(a,3f20.12)') ' lon, tlon, rlon = ', lon, tlon, rlon
                call endrun('sat_hist::get_indices: dyn lat is different than phys lat ')
             endif
             if ( abs(tlon-rlon)>1.e-3 ) then
                write(*,'(a,3f20.12)') ' lat, tlat, rlat = ', lat, tlat, rlat
                write(*,'(a,3f20.12)') ' lon, tlon, rlon = ', lon, tlon, rlon
          ...
          call phys_grid_find_col( rlat, rlon, phs_owners(i), ichk, icol )
       endif
