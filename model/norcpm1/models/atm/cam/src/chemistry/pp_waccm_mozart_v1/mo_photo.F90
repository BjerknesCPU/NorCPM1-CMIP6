...
contains
  ...
  subroutine photo_register
    ...
    call pbuf_add('IonRates' , 'physpkg', 1,pver,nIonRates, ion_rates_idx) ! Ionization rates for O+,O2+,N+,N2+,NO+ 
  ...
  subroutine photo_inti( xs_coef_file, xs_short_file, xs_long_file, rsf_file, &
    ...
    has_abs_columns : if( has_o2_col .or. has_o3_col ) then
       ...
       allocate( coldens(nlat,nlev,ntimes),stat=astat )
       if( astat /= 0 ) then
          write(iulog,*) 'photo_inti: coldens allocation error = ',astat
          call endrun
       end if
       if( has_o2_col ) then
          allocate( o2_exo_coldens(nlev,pcols,begchunk:endchunk,ntimes),stat=astat )
          if( astat /= 0 ) then
       ...
       if( has_o3_col ) then
          allocate( o3_exo_coldens(nlev,pcols,begchunk:endchunk,ntimes),stat=astat )
          if( astat /= 0 ) then
...
!------------------------------------------------------------------------------------------------------------
    if (ion_rates_idx>0) then
       pbuf(ion_rates_idx)%fld_ptr = 0._r8
    endif
...
!-----------------------------------------------------------------
    ...
    col_loop : do i = 1,ncol
       ...
       daylight : if( sza >= 0._r8 .and. sza < max_zen_angle ) then
          ...
          if (ion_rates_idx>0) then
             pbuf(ion_rates_idx)%fld_ptr(1,i,pver:1:-1,lchnk, 1:nIonRates) = esfact * euv_prates(:,1:nIonRates) 
          endif
