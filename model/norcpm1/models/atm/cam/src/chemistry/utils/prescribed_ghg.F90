...
!-------------------------------------------------------------------
!-------------------------------------------------------------------
  subroutine prescribed_ghg_adv( state, pbuf )
...
!$OMP PARALLEL DO PRIVATE (C, NCOL, OUTDATA, TO_MMR)
       do c = begchunk,endchunk
          ...
          pbuf(fields(i)%pbuf_ndx)%fld_ptr(1,:ncol,:,c,1) = to_mmr(:ncol,:) * pbuf(fields(i)%pbuf_ndx)%fld_ptr(1,:ncol,:,c,1)

          outdata(:ncol,:) = pbuf(fields(i)%pbuf_ndx)%fld_ptr(1,:ncol,:,c,1)
          call outfld( fields(i)%fldnam, outdata(:ncol,:), ncol, state(c)%lchnk )
       enddo
    enddo
