...
!    select case ( to_upper(data_type) )
    ...
    flds_loop: do f = 1,mxnflds
       ...
       ! determine if the field has a vertical diminsion
       ...
       if ( .not. file%in_pbuf ) then
          ...
          flds(f)%pbuf_ndx = pbuf_get_fld_idx(flds(f)%fldnam, failcode=-1 )
       endif
...
!-----------------------------------------------------------------------
  subroutine advance_trcdata( flds, file, state, pbuf )
    ...
    type(pbuf_fld),optional, intent(inout) :: pbuf(pbuf_size_max)

    real(r8) :: data_time

    if (file%in_pbuf .and. .not.present(pbuf)) then
       call endrun('advance_trcdata called with file%in_pbuf set to true and optional pbuf argument not present for : '&
                   //trim(file%curr_filename))
    endif
    ...
    if (present(pbuf)) then
       call interpolate_trcdata( state, flds, file, pbuf=pbuf )
    else
...
!-------------------------------------------------------------------
  subroutine get_fld_data( flds, field_name, data, ncol, lchnk, pbuf )
    ...
    type(pbuf_fld), optional, intent(in) :: pbuf(pbuf_size_max)
    ...
    do f = 1, nflds
       if ( trim(flds(f)%fldnam) == trim(field_name) ) then
          if ( flds(f)%pbuf_ndx>0 ) then
             if (.not.present(pbuf)) then
                call endrun('get_fld_data called without optional pbuf arg while fields are declared in phys buffer')
             endif
             data(:ncol,:) = pbuf(flds(f)%pbuf_ndx)%fld_ptr(1,:ncol,:,lchnk,1)
          else
...
!------------------------------------------------------------------------------
  ...
  subroutine interpolate_trcdata( state, flds, file, pbuf )
...
!$OMP PARALLEL DO PRIVATE (C, NCOL, F)
    ...
    fld_loop: do f = 1,nflds
       ...
       if (flds(f)%pbuf_ndx>0) then
          data_out => pbuf(flds(f)%pbuf_ndx)%fld_ptr(1,:,:,:,1)
       else
          data_out => flds(f)%data(:,:,:)
       endif

!$OMP PARALLEL DO PRIVATE (C, NCOL, PS, I, K, PIN, DATAIN, MODEL_Z)
       do c = begchunk,endchunk
          ncol = state(c)%ncol
          if (file%alt_data) then
             ...
             do i = 1,ncol
                ...
                call rebin( file%nlev, pver, file%ilevs, model_z, datain(i,:), data_out(i,:,c+chnk_offset) )
             enddo
             ...
             if (flds(f)%srf_fld) then
                ...
                if ( file%top_bndry ) then
                   call vert_interp_ub(ncol, file%nlev, file%levs,  datain(:ncol,:), data_out(:ncol,:,c+chnk_offset) )
                else if(file%conserve_column) then
                   call vert_interp_mixrat(ncol,file%nlev,pver,state(c)%pint, &
                        datain, data_out(:,:,c+chnk_offset), &
                        file%p0,ps,file%hyai,file%hybi)
                else
                   call vert_interp(ncol, file%nlev, pin, state(c)%pmid, datain, data_out(:,:,c+chnk_offset) )
                endif
             endif
