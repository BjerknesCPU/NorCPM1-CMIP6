...
!-------------------------------------------------------------------
!-------------------------------------------------------------------
  subroutine prescribed_ozone_adv( state, pbuf )
...
!$OMP PARALLEL DO PRIVATE (C, NCOL, OUTDATA, TO_MMR)
    do c = begchunk,endchunk
       ...
       pbuf(fields(1)%pbuf_ndx)%fld_ptr(1,:ncol,:,c,1) = to_mmr(:ncol,:) * pbuf(fields(1)%pbuf_ndx)%fld_ptr(1,:ncol,:,c,1)

       outdata(:ncol,:) = (amass/molmass)*pbuf(fields(1)%pbuf_ndx)%fld_ptr(1,:ncol,:,c,1) ! vmr
