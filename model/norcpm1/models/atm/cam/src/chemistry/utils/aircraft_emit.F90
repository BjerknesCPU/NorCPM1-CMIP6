...
!         forcings_air(m)%file%curr_filename    = spc_fname(m)
  ...
  subroutine aircraft_emit_adv( state, pbuf )
!-------------------------------------------------------------------
...
! C.-C. Chen
   ...
   !-------------------------------------------------------------------
    do m = 1, aircraft_cnt
       call advance_trcdata( forcings_air(m)%fields, forcings_air(m)%file, state, pbuf=pbuf)
...
!                end do
             ...
             pbuf(forcings_air(m)%fields(i)%pbuf_ndx)%fld_ptr(1,:ncol,:,c,1) = to_mmr(:ncol,:) * &
                  pbuf(forcings_air(m)%fields(i)%pbuf_ndx)%fld_ptr(1,:ncol,:,c,1) ! mmr

             call outfld( forcings_air(m)%fields(i)%fldnam, &
                  pbuf(forcings_air(m)%fields(i)%pbuf_ndx)%fld_ptr(1,:ncol,:,c,1), ncol, state(c)%lchnk )
