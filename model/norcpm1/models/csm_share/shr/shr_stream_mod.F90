!===============================================================================
! SVN $Id: shr_stream_mod.F90 28706 2011-05-27 19:59:04Z fischer $
! SVN $URL: https://svn-ccsm-models.cgd.ucar.edu/csm_share/branch_tags/cesm1_0_4_rel_tags/cesm1_0_4_n02_share3_110527/shr/shr_stream_mod.F90 $
!===============================================================================
!BOP ===========================================================================
!
...
!          rc = 1
      ...
      do n = 1,num
         ...
         call shr_cal_advDate(offin,'seconds',din,sin,dout,sout,calendar)
...
!         read(nUnit) strm(k)%file(n)%name       ! the file name
         read(nUnit) inpcl                      ! the file name
         read(nUnit) strm(k)%file(n)%haveData   ! has t-coord data been read in?
         read(nUnit) strm(k)%file(n)%nt         ! size of time dimension
         nt = strm(k)%file(n)%nt
         if (strm(k)%file(n)%haveData) then     ! ie. if arrays have been allocated
            if (trim(inpcl) /= trim(strm(k)%file(n)%name)) then
               write(s_logunit,F05) "ERROR in stream filenames ",k,n,' ',trim(inpcl),' ',trim(strm(k)%file(n)%name)
               abort=.true.
            endif
