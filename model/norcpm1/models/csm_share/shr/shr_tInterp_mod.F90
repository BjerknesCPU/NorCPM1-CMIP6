!===============================================================================
! SVN $Id: shr_tInterp_mod.F90 28565 2011-05-20 19:24:42Z fischer $
! SVN $URL: https://svn-ccsm-models.cgd.ucar.edu/csm_share/branch_tags/cesm1_0_4_rel_tags/cesm1_0_4_n02_share3_110527/shr/shr_tInterp_mod.F90 $
!===============================================================================
!BOP ===========================================================================
!
...
!EOP
   ...
   integer(SHR_KIND_I8)   :: itime1      ! elapsed seconds for 1 (lower-bound)
   integer(SHR_KIND_I8)   :: itime2      ! elapsed seconds for 2 (upper-bound)
   integer(SHR_KIND_I8)   :: itimeIn     ! elapsed seconds for model date
   integer(SHR_KIND_IN)   :: eday        ! elapsed days
   integer(SHR_KIND_IN)   :: edayin      ! elapsed days for model date
...
!-------------------------------------------------------------------------------
   ...
   call shr_cal_date2eday(Din, edayin,calendar)
   etimein = real(edayin,SHR_KIND_R8) + real(sin,SHR_KIND_R8)/shr_const_cDay
   itimein = int(edayin-edayin,SHR_KIND_I8)*spd + int(sin-sin,SHR_KIND_I8)

   call shr_cal_date2eday(D1, eday,calendar)
   ...
   call shr_cal_date2eday(D2, eday,calendar)
...
!  write(s_logunit,*) subName,' ITIME ',itime1,itime2,itimein
   ...
   if (itime2 < itime1) then
     if (s_loglev > 0) write(s_logunit,F01) ' ERROR: itime2 < itime1 D=',D1,D2
     if (s_loglev > 0) write(s_logunit,F01) ' ERROR: itime2 < itime1 i=',itime2,itime1
     lrc = 1
   ...
   elseif (trim(lalgo) == 'nearest') then
     if (abs(itimein-itime1) <= abs(itime2-itimein)) then
       f1 = c1
     else
       f1 = c0
     endif
   elseif (trim(lalgo) == 'linear') then
     ...
     if (itime2 < itimein .or. itime1 > itimein) then
       write(s_logunit,F02) ' ERROR illegal linear times: ',itime1,itimein,itime2
       lrc = 1
...
! tcx tcraig repro annual cycle, used computation below
!       f1 = real((itime2-itimein),shr_kind_r8)/real((itime2-itime1),shr_kind_r8)
     endif
   else
...
!EOP
   ...
   integer(SHR_KIND_I8) :: reday1,reday2      ! LB,UB elapsed seconds
   integer(SHR_KIND_I8) :: n                  ! number of time-samples in t-avg
   integer(SHR_KIND_IN) ::  eday              ! time sample's elapsed days, integer
   integer(SHR_KIND_I8) :: reday,reday0       ! time sample's elapsed seconds
...
!-------------------------------------------------------------------------------
   ...
   !--- get LB & UB dates ---
   call shr_cal_date2eday(ymd1,eday1,calendar)
   call shr_cal_date2eday(ymd2,eday2,calendar)
   reday1 = eday1*spd + tod1
   reday2 = eday2*spd + tod2
   if (reday1 > reday2) call shr_sys_abort(subname//'ERROR: lower-bound > upper-bound')

   if (mod(int(reday2-reday1,SHR_KIND_IN),ldt) /= 0) then
      ldt = (reday2-reday1)/((reday2-reday1)/ldt+1)
   endif
   ...
   do while( reday < reday2) ! mid-interval t-steps thru interval [LB,UB]
      ...
      call shr_cal_date2eday(ymd ,eday,calendar) ! convert to elapsed days
      reday = eday*spd + tod
